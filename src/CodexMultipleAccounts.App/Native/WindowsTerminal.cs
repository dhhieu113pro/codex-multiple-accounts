...
namespace CodexMultipleAccounts.App.Native;
...
internal static partial class WindowsTerminal
{
    ...
    internal static bool IsSupported
    {
        get
        {
            ...
            return (architecture == Architecture.X64 || architecture == Architecture.Arm64) && File.Exists(GetLibraryPath());
        }
    }
    ...
    internal static string GetLibraryPath()
    {
        ...
        return string.IsNullOrEmpty(rid)
            ? string.Empty
            : Path.Combine(AppContext.BaseDirectory, "native-terminal", rid, "Microsoft.Terminal.Control.dll");
    }
    ...
    internal static void EnsureResolver()
    {
        ...
        NativeLibrary.SetDllImportResolver(
            ...
            static (name, _, _) =>
            {
                ...
                return !string.IsNullOrEmpty(file) && File.Exists(file) && NativeLibrary.TryLoad(file, out var handle)
                    ? handle
                    : IntPtr.Zero;
            });
    }
    ...
    [UnmanagedCallConv(CallConvs = [typeof(CallConvStdcall)])]
    internal static partial void TerminalSendKeyEvent(nint terminal, ushort vkey, ushort scanCode, ushort flags, byte keyDown);

    [LibraryImport(LibraryName, EntryPoint = "TerminalSendCharEvent")]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvStdcall)])]
    internal static partial void TerminalSendCharEvent(nint terminal, ushort ch, ushort scanCode, ushort flags);
