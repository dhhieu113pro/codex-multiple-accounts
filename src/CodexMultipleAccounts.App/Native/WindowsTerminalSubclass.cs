...
internal sealed class WindowsTerminalSubclass : IDisposable
{
    ...
    internal static WindowsTerminalSubclass Attach(nint hwnd, Func<uint, nuint, nint, nint?> handler)
    {
    ...
    private WindowsTerminalSubclass(nint hwnd, Func<uint, nuint, nint, nint?> handler)
    {
        _hwnd = hwnd;
    ...
    public void Dispose() => DetachCore(removeSubclass: true);
    ...
    private static nint SubclassThunk(nint hwnd, uint message, nuint wParam, nint lParam, nuint subclassId, nuint refData)
    {
