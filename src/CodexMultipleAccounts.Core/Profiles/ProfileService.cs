...
{
    private readonly string _root; private readonly string _defaultHome; private readonly string _catalog;
    public ProfileService(string root, string defaultHome) { _root=Path.GetFullPath(root); _defaultHome=Path.GetFullPath(defaultHome); _catalog=Path.Combine(_root,"profiles.json"); Directory.CreateDirectory(Path.Combine(_root,"profiles")); }
    public async Task<IReadOnlyList<CodexProfile>> ListAsync() => await LoadAsync();
    public async Task<CodexProfile> CreateAsync(string name) { if(string.IsNullOrWhiteSpace(name)) throw new ArgumentException("Profile name is required.",nameof(name)); var p=new CodexProfile(Guid.NewGuid(),name.Trim(),Path.Combine(_root,"profiles",Guid.NewGuid().ToString("N"),"codex-home"),null,false); Directory.CreateDirectory(p.CodexHome); var all=await LoadAsync(); all.Add(p); await SaveAsync(all); return p; }
    public async Task<CodexProfile> ImportDefaultAsync(string name) { var p=await CreateAsync(name); if(Directory.Exists(_defaultHome)) CopyDirectory(_defaultHome,p.CodexHome); return p; }
